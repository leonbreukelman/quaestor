...
from quaestor.analysis.linter import LintResult, LinterConfig, StaticLinter
...
class AnalysisReport:
    ...
    def summary(self) -> dict[str, Any]:
        ...
        return {
            ...
            "tools_found": (
                len(self.workflow_analysis.tools_detected)
                if self.workflow_analysis else 0
            ),
            "states_found": (
                len(self.workflow_analysis.states_detected)
                if self.workflow_analysis else 0
            ),
        }
        ...
        if self.workflow_analysis:
            result["workflow"] = {
                ...
                "states": [
                    ...
                    for s in self.workflow_analysis.states_detected
                ],
        ...
        try:
            ...
            if report.parsed.has_syntax_errors:
                report.errors.append(
                    f"Syntax errors in {file_name}: {report.parsed.syntax_errors}"
                )
