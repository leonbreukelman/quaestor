...
from quaestor.analysis.parser import ParsedCode, FunctionDef, ClassDef
    ...
    def _generate_recommendations(
        ...
        if undocumented:
            recommendations.append(
                f"Add docstrings to {len(undocumented)} undocumented tool(s)"
            )
    ...
    def to_agent_workflow(self, analysis: WorkflowAnalysis) -> AgentWorkflow:
        ...
        tools = [
            ToolDefinition(
                ...
                parameters=[
                    ToolParameter(name=p, type="Any", required=True)
        ...
        states = [
            StateDefinition(
                name=s.name,
                state_type=StateType(s.type) if s.type in ["initial", "intermediate", "terminal"] else StateType.INTERMEDIATE,
