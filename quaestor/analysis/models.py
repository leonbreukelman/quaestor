...
class ToolDefinition(BaseModel):
    ...
    side_effects: list[str] = Field(
        ...
        description="Side effects (e.g., 'modifies_database', 'sends_email')"
    )
    requires_confirmation: bool = Field(
        default=False,
        description="Whether tool requires user confirmation"
    )
...
class StateDefinition(BaseModel):
    ...
    variables: list[StateVariable] = Field(
        default_factory=list,
        description="Variables in this state"
    )
    allowed_tools: list[str] = Field(
        default_factory=list,
        description="Tools that can be called in this state"
    )
...
class Invariant(BaseModel):
    ...
    severity: str = Field(
        default="error",
        description="Severity if violated: error, warning, info"
    )
    category: str = Field(
        default="correctness",
        description="Category: correctness, safety, security, performance"
    )
...
class AgentWorkflow(BaseModel):
    ...
    states: list[StateDefinition] = Field(
        default_factory=list,
        description="Possible agent states"
    )
    ...
    entry_points: list[EntryPoint] = Field(
        default_factory=list,
        description="Ways to invoke the agent"
    )
    ...
    analyzer_version: str | None = Field(
        default=None,
        description="Version of analyzer used"
    )
    
    # Governance integration
    governance_rules: list[str] = Field(
        default_factory=list,
        description="Applicable governance rule IDs from Smactorio"
    )
