...
class QuaestorOptimizer:
    ...
    def workflow_analysis_metric(example, prediction, trace=None):
        ...
        tool_recall = len(set(t.name for t in spec.tools) & set(t.name for t in gold.tools)) / len(gold.tools)
        state_recall = len(set(s.name for s in spec.states) & set(s.name for s in gold.states)) / len(gold.states)
    ...
    def test_effectiveness_metric(example, prediction, trace=None):
        ...
        tests_that_found_bugs = sum(
            1 for bug in example.known_bugs 
    ...
    def investigation_efficiency_metric(example, prediction, trace=None):
        ...
        false_positive_penalty = sum(1 for f in prediction.findings if f.category not in example.valid_findings) * 0.1
    ...
    def bootstrap_analyzer(self, training_examples):
        ...
        optimizer = BootstrapFewShot(
            metric=self.workflow_analysis_metric,
